...
credit: https://medium.com/@santhanu/batch-upsert-pyspark-dataframe-into-postgres-tables-with-error-handling-using-psycopg2-and-asyncpg-59f08aa020b0
"""
...
if __name__ == '__main__':
    spark = SparkSession \
        ...
        .config("spark.jars", "/Users/omarnour/Downloads/postgresql-42.5.0.jar") \
        .config("spark.executor.memory", "16g") \
        .getOrCreate()

    src_df = spark.read \
        .format("jdbc") \
        .option("url", "jdbc:postgresql://localhost:5432/raw_db") \
    ...
    src_df.createOrReplaceTempView("src_order_lines")

    df_data = spark.sql("""
    SELECT order_id, product_id, quantity qty, unit_price price, batch_id, 'spark job' created_by 
    FROM src_order_lines limit 200000;
    """)
    upsert_spark_df_to_postgres(dataframe_to_upsert=df_data
                                , table_name='public.order_details'
                                , table_unique_key=['order_id', 'product_id']
